      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-sm text-gray-400 mt-1">
            <Link to="/lpos/new" className="text-indigo-600 font-medium hover:underline">Create your first LPO</Link>
          </p>
        </div>
      ) : (
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(lpo => (
                  <tr key={lpo.id} className="hover:bg-gray-50/70 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <Link to={`/lpos/${lpo.id}`}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-indigo-200 text-indigo-600 text-xs font-semibold rounded-lg hover:bg-indigo-50 transition-colors">
