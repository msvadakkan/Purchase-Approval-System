...
export default function QuoteComparison() {
  ...
  useEffect(() => {
    api.get(`/tenders/${id}/comparison`)
      ...
      .catch(() => navigate(`/requirements/${id}`))
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <button onClick={() => navigate(-1)} className="text-indigo-600 text-sm hover:underline mb-4 block">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto">
                    ...
                    return (
                      <td key={q.id}
                        className={`px-5 py-3 text-sm text-center font-medium ${
                          ...
                          isFastest && key === 'delivery_days' ? 'text-blue-700' :
                          'text-gray-700'
          ...
          <div className="p-5 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
            <p className="text-sm text-gray-500">
              Recommended: <strong className="text-gray-800">{quotes[lowestIdx]?.vendor_name}</strong> — lowest total at <strong className="text-green-600">{Number(quotes[lowestIdx]?.total_amount).toLocaleString()} {quotes[lowestIdx]?.currency}</strong>
            </p>
            <Link to={`/requirements/${id}`} className="text-indigo-600 text-sm hover:underline">
              Back to Tender
            </Link>
          </div>
        </div>
