<?php
/**
 * Magenta Investments LLC — Procurement System Installer
 * Multi-step setup wizard: DB → Azure AD → SMTP → Install
...
function test_smtp(string $host, int $port, string $user, string $pass): array {
    try {
        ...
        fwrite($fp, "EHLO magenta-investments.com\r\n"); fgets($fp, 512);
        fwrite($fp, "STARTTLS\r\n");
        $tls = fgets($fp, 512);
...
if ($step === 'step4' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$adminName || !$adminEmail || !$adminPass) {
        ...
        $step = 'step3'; // advance to Azure AD step
    }
}
...
if ($step === 'install' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    $envData = [
        ...
        'AZURE_AD_TENANT_ID'    => $d['azure_tenant']    ?? '',
        'AZURE_AD_CLIENT_ID'    => $d['azure_client_id'] ?? '',
        'AZURE_AD_CLIENT_SECRET'=> $d['azure_secret']    ?? '',
    ...
    if (!isset($d['admin_hash']) || !$d['admin_hash']) {
        ...
        try {
            ...
            ob_end_clean();
            $info[] = 'Database seeded (approval levels, company, indexes).';
...
$STEPS = [
    ...
    'step3'        => ['num' => 4, 'label' => 'Microsoft 365 SSO'],
...
<head>
  ...
  <title>Magenta Investments — System Setup</title>
...
<div class="page">
  ...
  <aside class="sidebar">
    <div class="brand">
      ...
      <div class="company">Magenta Investments</div>
    ...
    <nav class="steps-nav">
      ...
      $stepDefs = [
        ...
        ['key' => 'step3',        'num' => 4, 'icon' => '🔑', 'label' => 'Microsoft 365 SSO'],
  ...
  <div class="main">
      ...
      <div class="card-body">
        <form method="POST">
          ...
          <div class="field">
            ...
            <input name="app_url" value="<?= htmlspecialchars($d['app_url']) ?>" placeholder="https://magenta-investments.com" required />
      ...
      <div class="card-body">
        ...
        <form method="POST">
          ...
          <div class="field">
            ...
            <input type="email" name="admin_email" value="<?= htmlspecialchars($d['admin_email'] ?? '') ?>" placeholder="admin@magenta-investments.com" required autocomplete="email" />
          ...
          <div class="btn-row">
            ...
            <button type="submit" class="btn btn-primary">Continue → Microsoft 365 SSO</button>
          </div>
        </form>
      </div>
    </div>

    <!-- ── STEP 4: Azure AD / Microsoft 365 SSO ─────────────────────────── -->
    <?php elseif ($step === 'step3'): ?>
    <div class="card">
      <div class="card-header">
        <div class="icon">🔑</div>
        <div>
          <h2>Microsoft 365 Single Sign-On</h2>
          <p>Allow staff to sign in using their @magenta-investments.com company accounts</p>
        </div>
      </div>
      <div class="card-body">
        <div class="info-box">
          📋 <strong>How to register your app in Azure:</strong><br />
          1. Go to <a href="https://portal.azure.com" target="_blank">portal.azure.com</a> → Azure Active Directory → App registrations → New registration<br />
          2. Name: <em>Magenta Procurement System</em><br />
          3. Supported account types: <em>Accounts in this organizational directory only</em><br />
          4. Redirect URI (Web): <code><?= htmlspecialchars(rtrim($d['app_url'] ?? '', '/')) ?>/api/ms-callback.php</code><br />
          5. After creating: copy <strong>Application (client) ID</strong> and <strong>Directory (tenant) ID</strong><br />
          6. Go to <em>Certificates &amp; secrets</em> → New client secret → copy the <strong>Value</strong>
        </div>
        ...
        <form method="POST">
          ...
          <div class="field">
            ...
            <input name="azure_tenant" class="mono" value="<?= htmlspecialchars($d['azure_tenant'] ?? '') ?>" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" />
            <div class="desc">Found on the Azure AD Overview page. Also called "Directory ID".</div>
          </div>
          <div class="grid-2">
            <div class="field">
              <label>Application (Client) ID</label>
              <input name="azure_client_id" class="mono" value="<?= htmlspecialchars($d['azure_client_id'] ?? '') ?>" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" />
            </div>
            <div class="field">
              <label>Client Secret Value</label>
              <input type="password" name="azure_secret" value="<?= htmlspecialchars($d['azure_secret'] ?? '') ?>" placeholder="••••••••••••••••" />
              <div class="desc">Copy immediately after creation — Azure won't show it again.</div>
            </div>
          </div>

          <hr class="sep" />
          <p style="font-size:.8rem;color:#6b7280;margin-bottom:1rem">
            After staff users sign in via Microsoft, they must also have an account created by the admin in the system with a matching email address.
          </p>

          <div class="btn-row">
            <a href="install.php?step=step3b" class="btn btn-outline">← Back</a>
            <button type="submit" class="btn btn-skip">Skip SSO (password login only)</button>
    ...
    <!-- ── STEP 5: SMTP / Microsoft 365 Mail ────────────────────────────── -->
    <?php elseif ($step === 'step5'): ?>
    <div class="card">
      <div class="card-header">
        <div class="icon">📧</div>
        <div>
          <h2>Microsoft 365 Mail Server</h2>
          <p>Configure SMTP to send approval notifications via Office 365</p>
        </div>
      </div>
      <div class="card-body">
        <div class="info-box">
          📋 <strong>Microsoft 365 SMTP prerequisites:</strong><br />
          1. In <a href="https://admin.microsoft.com" target="_blank">Microsoft 365 Admin Center</a> → Users → Active users → select the mailbox user<br />
          2. Mail tab → Manage email apps → enable <strong>Authenticated SMTP</strong><br />
          3. If MFA is enabled, create an <strong>App Password</strong> at <a href="https://mysignins.microsoft.com/security-info" target="_blank">mysignins.microsoft.com</a><br />
          4. Tenant admin: Security &amp; Compliance → Policies → <strong>SMTP AUTH</strong> must be allowed
        </div>
        ...
        <form method="POST">
          ...
          <div class="field">
            ...
            <input type="email" name="smtp_user" value="<?= htmlspecialchars($d['smtp_user'] ?? '') ?>" placeholder="notifications@magenta-investments.com" />
          </div>
          <div class="field">
          ...
          <div class="field">
            ...
            <input type="email" name="smtp_from" value="<?= htmlspecialchars($d['smtp_from'] ?? $d['smtp_user'] ?? '') ?>" placeholder="notifications@magenta-investments.com" />
          ...
          <div class="btn-row">
            <a href="install.php?step=step3" class="btn btn-outline">← Back</a>
    ...
    <div class="card">
      ...
      <div class="card-body">
        <table class="cred-table">
          ...
          <tr><td>Azure AD SSO</td><td><?= !empty($d['azure_tenant']) ? '✅ Configured' : '⏭️ Skipped' ?></td></tr>
    ...
    <div class="card">
      <div class="card-body">
        <div class="done-wrap">
          ...
          <p>Your Magenta Investments Procurement System is ready.<br />Sign in with the admin account below to get started.</p>
