...
export default function RequirementDetailPage() {
  ...
  return (
    <div className="p-6">
      <button onClick={() => router.back()} className="text-indigo-600 text-sm hover:underline mb-5 block">← Back to Requirements</button>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {quotes.length >= 2 && (
                <Link href={`/requirements/${id}/comparison`}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 text-left">
                      {['Vendor', 'Total (AED)', 'Unit Price', 'Delivery', 'Validity'].map(h => (
                        <th key={h} className="pb-2 text-xs font-semibold text-gray-400 uppercase">{h}</th>
                      ))}
                    </tr>
                  </thead>
