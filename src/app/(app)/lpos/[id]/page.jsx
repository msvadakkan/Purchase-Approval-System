...
export default function LPODetailPage() {
  ...
  useEffect(() => {
    api.get(`/lpos/${id}`).then(({ data }) => setLpo(data)).finally(() => setLoading(false))
  }, [id])
  ...
  const sc = STATUS_COLORS[lpo.status] ?? STATUS_COLORS.draft

  return (
    <>
      <style>{`
        @media print { .no-print { display: none !important; } body { margin: 0; } .lpo-page { box-shadow: none !important; border: none !important; padding: 0 !important; } }
      ...
      <div className="no-print p-4 sm:p-6 pb-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={`px-3 py-1 rounded-full text-xs font-bold capitalize ${sc.bg} ${sc.text}`}>{lpo.status}</span>
            {lpo.status === 'draft' && (
            ...
            <button onClick={() => window.print()}
              className="flex items-center gap-1.5 px-4 py-1.5 bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-700">
        ...
        <div className="lpo-page max-w-4xl mx-auto bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-indigo-900 to-indigo-700 text-white px-8 py-6">
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              ...
              <div className="text-right">
                ...
                {lpo.delivery_date && (
                  <p className="text-indigo-200 text-xs">Delivery: {new Date(lpo.delivery_date).toLocaleDateString('en-GB', { day:'2-digit', month:'short', year:'numeric' })}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
            <div className="px-8 py-5">
              ...
              {co.address && <div className="flex items-start gap-1.5 mt-1.5 text-sm text-gray-500"><MapPin className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-gray-400" /><span>{co.address}{co.city ? `, ${co.city}` : ''}{co.country ? `, ${co.country}` : ''}</span></div>}
              {co.phone && <div className="flex items-center gap-1.5 mt-1 text-sm text-gray-500"><Phone className="w-3.5 h-3.5 text-gray-400" /><span>{co.phone}</span></div>}
              {co.email && <div className="flex items-center gap-1.5 mt-1 text-sm text-gray-500"><Mail className="w-3.5 h-3.5 text-gray-400" /><span>{co.email}</span></div>}
              {co.website && <div className="flex items-center gap-1.5 mt-1 text-sm text-gray-500"><Globe className="w-3.5 h-3.5 text-gray-400" /><span>{co.website}</span></div>}
            </div>
            <div className="px-8 py-5 bg-gray-50/50">
              ...
              {vd.address && <div className="flex items-start gap-1.5 mt-1.5 text-sm text-gray-500"><MapPin className="w-3.5 h-3.5 mt-0.5 flex-shrink-0 text-gray-400" /><span>{vd.address}</span></div>}
              {vd.contact_number && <div className="flex items-center gap-1.5 mt-1 text-sm text-gray-500"><Phone className="w-3.5 h-3.5 text-gray-400" /><span>{vd.contact_number}</span></div>}
              {vd.email && <div className="flex items-center gap-1.5 mt-1 text-sm text-gray-500"><Mail className="w-3.5 h-3.5 text-gray-400" /><span>{vd.email}</span></div>}
              {vd.vat_number && <p className="text-xs text-gray-400 mt-1">TRN: {vd.vat_number}</p>}
              {vd.sales_person && <p className="text-xs text-gray-400 mt-0.5">Attn: {vd.sales_person}</p>}
            </div>
          </div>
                ...
                <tbody>
                  {(lpo.items ?? []).map((item, i) => (
                    <tr key={i} className={`border-b border-gray-50 ${i % 2 === 1 ? 'bg-gray-50/40' : ''}`}>
                      ...
                      <td className="py-3 text-right text-gray-700">{Number(item.unit_price).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                      <td className="py-3 text-right font-semibold text-gray-900">{Number(item.total).toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="mt-4 flex justify-end">
              <div className="w-72 space-y-1.5">
                <div className="flex justify-between text-sm text-gray-600 py-1">
                  <span>Subtotal</span><span className="font-semibold">{fmtAmt(lpo.subtotal, currency)}</span>
                </div>
                <div className="flex justify-between text-sm text-gray-600 py-1">
                  <span>VAT ({lpo.vat_rate ?? 5}%)</span><span className="font-semibold">{fmtAmt(lpo.vat_amount, currency)}</span>
                </div>
                <div className="flex justify-between items-center bg-indigo-600 text-white px-4 py-3 rounded-xl mt-2">
                  <span className="font-bold text-sm">TOTAL AMOUNT</span>
                  <span className="font-black text-base">{fmtAmt(lpo.total_amount, currency)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 border-t border-gray-100 px-8 py-3 text-center">
            <p className="text-xs text-gray-400">This is a computer-generated document. · {co.name} · {co.email}</p>
          </div>
        </div>
      </div>
    </>
