...
import { FileText, Clock, CheckCircle2, XCircle, TrendingUp, Users, PlusCircle, ClipboardCheck, ArrowRight, Banknote } from 'lucide-react'
  ...
  return (
    <div className="p-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatCard label="Total Requests" value={adminStats?.total_requests ?? requests.length} Icon={FileText}     iconBg="bg-indigo-50" iconColor="text-indigo-600" />
        <StatCard label="Pending"        value={adminStats?.pending  ?? pending}                Icon={Clock}        iconBg="bg-yellow-50" iconColor="text-yellow-600" />
        <StatCard label="Approved"       value={adminStats?.approved ?? approved}               Icon={CheckCircle2} iconBg="bg-green-50"  iconColor="text-green-600" />
        <StatCard label="Rejected"       value={adminStats?.rejected ?? rejected}               Icon={XCircle}      iconBg="bg-red-50"    iconColor="text-red-600" />
      </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-0.5">AED {Number(adminStats.total_approved_amount).toLocaleString()}</p>
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex items-center gap-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          ...
          <Link href="/requests" className="text-indigo-600 text-sm font-medium hover:underline flex items-center gap-1">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400">
            No requests yet.{' '}
            <Link href="/requests/new" className="text-indigo-600 font-medium hover:underline">Create one</Link>.
          </div>
        ) : (
              ...
              <Link key={req.id} href={`/requests/${req.id}`}
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-gray-900 truncate group-hover:text-indigo-600">{req.title}</p>
                    <p className="text-xs text-gray-400">{req.category} · {new Date(req.created_at).toLocaleDateString()}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 ml-4 flex-shrink-0">
                  <span className="text-sm font-bold text-gray-700">AED {Number(req.amount).toLocaleString()}</span>
